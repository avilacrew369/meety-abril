import AttendanceToogleBotton from "@/src/features/meetis/components/AttendanceToogleBotton";
import { DynamicMeetiLocation } from "@/src/features/meetis/components/DinamicMeetiLocation";
...
export default async function MeetiPage(props: PageProps<'/meetis/[id]'>) {
    ...
    return (
        <>
            ...
            {!session?.user && (
                <p className="font-bold border  border-orange-400 p-2 " >Confirma tu asistencia, obteniendo una Cuenta, Es Gratis !</p>
            )}
            {meeti.permission && !meeti.context.isAdmin && (
                    <AttendanceToogleBotton
                        meetiId={meeti.data.id}
                        permissions={meeti.permission}
                    />
                )}
                </div>
            ...
            <main className="max-w-7xl mx-auto grid grid-cols-1 gap-5 lg:grid-cols-3 p-5 lg:px-0 mt-10">
                <section className="lg:col-span-2">
                    <Image
                        ...
                        alt={`Imangen de Meeti ${meeti.data.title}`}
                        width={800}
                        height={600}
                        priority
                    />
